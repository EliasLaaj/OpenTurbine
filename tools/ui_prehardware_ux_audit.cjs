...
(async () => {
  ...
  try {
    ...
    await reset(page);
    await goto(page, 'system.html', '#system-maintenance-group');
    await scenario(page, 'startup');
    await page.reload();
    await page.waitForSelector('#cfg-restore-btn', {state:'attached'});
    await page.waitForFunction(() => document.getElementById('state-factory-reset')?.textContent.includes('STARTUP'));
    ...
    await goto(page, 'system.html', '#system-maintenance-group');
    assert.match(await page.locator('body').evaluate(() => systemBackupConfig.toString()), /Download started.*Confirm the file appears in Downloads/s);
    await goto(page, 'tools.html', '#tool-area');
