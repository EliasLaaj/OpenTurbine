...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => document.querySelector('#appearance-picker .ot-tile'));
    await page.locator('#system-appearance details > summary').click();
    ...
    assert.equal(await page.locator('#system-appearance').isVisible(), true);
    ...
    await page.evaluate(() => document.querySelector('.save-bar').classList.remove('is-dirty'));
    assert.equal(await page.locator('#card-CONFIG_BACKUP').count(), 1);
    ...
    await page.goto(`${base}/tools.html`);
    assert.equal(await page.locator('#card-CONFIG_BACKUP').count(), 0);
