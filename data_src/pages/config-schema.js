...
const SCHEMA = [
  ...
  { title: 'Gradual Fuel Limit Protection', id: 'gradual-protection', sectionNote:'Optional non-emergency fuel reduction as a measured value approaches its configured boundary. Independent hard shutdown limits remain active and separate.', fields: [
    ...
    { key:'pb_min', path:['throttle','pullback_min_pct'], label:'Minimum Fuel During Gradual Protection (%)', desc:'Lowest fuel/throttle output allowed by gradual protection. Emergency protection may still command less.', step:1, min:0, max:100 },
    { key:'pb_str', path:['throttle','pullback_strength'], label:'Limit-Protection Strength', desc:'How strongly fuel is reduced near a limit. 1.0 is normal; higher is more aggressive and may cause oscillation.', step:0.1, min:0, max:5 },
    { key:'rl_mode', path:['throttle','rpm_limiter_mode'], label:'Gradual Limit-Protection Method', type:'select', options:[{v:0,l:'React to current readings'},{v:1,l:'Predict approaching limits'}], desc:'Reactive control uses current N1/N2/P1/P2/EGT/torque readings. Predictive control also uses each signal rate of change to estimate where it is heading and reduce fuel before an overshoot.' },
    { key:'rl_look', path:['throttle','pullback_lookahead_ms'], label:'Limit Prediction Time (ms)', desc:'How far ahead Advanced mode projects speed, pressure, selected TOT/TIT, and torque. Typical starting point: 1000-2000 ms.', step:100, min:0 },
  ...
  { title: 'Afterburner — Ignition Conditions', id: 'ab-cfg-section', fields: [
    ...
    { key:'ab_tt', path:['afterburner','throttle_threshold'], label:'Throttle Trigger %', scale:100, desc:'Throttle demand percent that triggers AB when source is set to Throttle.', step:0.05, min:0, max:1 },
  ]},
